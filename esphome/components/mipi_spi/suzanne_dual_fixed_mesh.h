...
PerspectiveStats render_dual_suzanne_fixed_mesh(DisplayT *display, uint8_t phase_x, uint8_t phase_y) {
  ...
  // Near/lower object. It is offset mostly along the 320-pixel screen axis,
  // with a small X offset so the silhouettes overlap without being perfectly
  // concentric. This one also exercises the generic Z-rotation path.
  ...
  near_params.translate_x = 18;
  near_params.translate_y = -105;
  ...
  // Far/upper object, intentionally drawn second. It should still disappear
  ...
  far_params.translate_x = -18;
  far_params.translate_y = 115;
